...
def b():
    ...
    for i in liczby:
        if len(i) > 1:
            a = int(i)
            b = int(i[::-1])
            if czypierwsze(a) and czypierwsze(b):
                if a < b:
                    if [a,b] not in parypierwsze:
                        parypierwsze.append([a, b])
                else:
                    if [b,a] not in parypierwsze:
                        parypierwsze.append([b, a])
    a = parypierwsze
    print(len(a))
    for i in range(len(parypierwsze)):
        minpara = parypierwsze[0]
        minwartosc = parypierwsze[0][0]
...
def c():
    ...
    for i in liczby:
        cyfra = i + i[::-1]
        wszystkie.append(int(cyfra))
    nwd = math.gcd(wszystkie[0],wszystkie[1])
    for i in range(2, len(wszystkie)):
        nwd = math.gcd(nwd, wszystkie[i])
    print(nwd)
